...
test.describe('ExamFlow Authentication Stress Test', () => {
  ...
  test('should login and complete exam successfully with matrix user', async ({ page }: { page: Page }) => {
    ...
    // 8. التأكد من إمكانية التسليم (يتم تفعيل زر التسليم بعد انتهاء ثلثي الوقت حسب التطبيق)
    const submitBtn = page.locator('.submit-btn');
    const isSubmitVisible = await submitBtn.isVisible();
    const isSubmitEnabled = isSubmitVisible && await submitBtn.isEnabled();

    if (isSubmitEnabled) {
      await submitBtn.click();
      console.log('📤 تم ضغط زر التسليم بنجاح!');
      await page.waitForTimeout(2000); // للانتظار حتى تسجل الطلبات
    } else {
      console.log('⏳ زر التسليم غير مفعل لأنه مقفل برمجياً (مرتبط بالوقت). محاكاة الحل تمت بالكامل.');
    }
